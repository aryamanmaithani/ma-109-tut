...
\begin{itemize}
	...
	\begin{soln}
		...
		\begin{equation*} 
			\left|f(x, y)\right| \le \left|x\right| + \left|y\right| \le \sqrt{x^2 + y^2}.
		\end{equation*}
		...
		\end{equation*}
		Thus, given any $\epsilon > 0,$ $\delta = \epsilon$ works in the definition of continuity.
	...
	\begin{soln}
		...
		\begin{align*} 
			\dfrac{\left|f(0 + h, 0 + k) - f(0, 0) - A\begin{bmatrix}
							h\\
							k
						\end{bmatrix}\right|}{\|(h, k)\|} &= \frac{f\left(0+h, 0+k\right)-f\left(0, 0\right)-0 h- 1k}{\sqrt{h^{2}+k^{2}}}\\
						&= \dfrac{k}{|k|} - \dfrac{k}{\sqrt{h^2 + k^2}}
		\end{align*}
		Note that along the curve $h = k$ with $(h, k) \neq (0, 0),$ we see that the above expression equals
		\begin{equation*} 
			\dfrac{k}{|k|} - \dfrac{k}{\sqrt{h^2 + k^2}} = \dfrac{k}{|k|} - \dfrac{k}{\sqrt{2k^2}} = \dfrac{k}{|k|}\left(1 - \dfrac{1}{\sqrt{2}}\right)
		\end{equation*}
		and the limit of \emph{that} does not exist as $k \to 0.$

		Thus, we see that the original limit (which was supposed to be $0$) also does not exist. Thus, $f$ is not differentiable at $(0, 0).$
	\end{soln}
\end{itemize}
\end{document}